...
class SccSmoother:
  def __init__(self, accel_gain, decel_gain, curvature_gain):
    ...
    self.sync_set_speed_while_gas_pressed = True # False
  ...
  def cal_acc(self, apply_accel, CS, clu11_speed, sm):
    ...
    if lead is None or lead.modelProb < 0.5:
      ...
      if 0. < d < -lead.vRel * (9. + cruise_gap) * 2. and lead.vRel < -1.:
        ...
        accel = op_accel * interp(clu11_speed, [50., 100.], [1.5, 1.0]) # 1.4, 1.2

    if accel > 0.:
      accel *= self.accel_gain * interp(clu11_speed, [30., 100.], [1.5, 1.25]) # 1.0
    else:
